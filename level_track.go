...
var runtimeLogLevel = zap.NewAtomicLevelAt(zap.DebugLevel)
var runtimeEnableTrackLevel atomic.Bool
...
var newTrackLevelEnabler = func() zapcore.LevelEnabler {
	return &trackLevelEnabler{
		level:       runtimeLogLevel,
		enableTrack: &runtimeEnableTrackLevel,
	}
}

type trackLevelEnabler struct {
	level       zap.AtomicLevel
	enableTrack *atomic.Bool
}

// Enabled track级日志不允许屏蔽
func (t *trackLevelEnabler) Enabled(level zapcore.Level) bool {
	if t.enableTrack.Load() && level == TrackLevel {
		return true
	}
